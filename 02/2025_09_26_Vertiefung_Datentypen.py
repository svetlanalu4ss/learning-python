...
# В Python не нужно заранее указывать тип переменной.
# type() → узнать тип переменной.
# isinstance() → проверить, является ли переменная нужного типа.
# Python умеет неявно преобразовывать типы при операциях (10/4 → float).
# Можно явно преобразовать тип с помощью int(), float(), str(), bool().
# Любое значение можно привести к булевому: пустое → False, непустое → True.
